...
class Pool(Resource):
    ...
    def apicall_with_insertdb(self, uri, cmd, method):
        try:
            ...
            return 'cannot change pool method'
    ...
    def post(self, pool_name):
        ...
        if args['method'] == 'change_members':
            cmd = command.pools + '/' + pool_name + command.change_members
            uri = cmd.split("-d")[0].strip()
            cmd = json.loads(cmd.split("-d")[-1].split("'")[1])

            if Valid.valid_args(args['data'], cmd):
                #return 'uri:', uri, 'cmd:', cmd, 'args:', args['data']
                return self.apicall_with_insertdb(uri, cmd, 'PATCH')
            else:
                return 'You must request with members'
        else:
